...
3. สวนเทรนด์ได้เฉพาะเมื่อมี reversal pattern ที่ body ใหญ่กว่า 0.5×ATR + เกิดที่แนวรับ/ต้าน Fibo หรือ 24H High/Low → confidence สูงสุดแค่ MEDIUM
4. RSI oversold ใน downtrend ที่ไม่มี reversal pattern = falling knife → WAIT
5. ADX < 10 → ตลาด Sideway ไร้ทิศทางชัดเจน → WAIT
...
{"reasoning":"1 บรรทัด","action":"BUY|SELL|WAIT","confidence":"HIGH|MEDIUM|LOW"}`;
        ...
        const prompt = `
            ...
            - Alignment H1↔M15: ${trendAligned ? '✅ สอดคล้อง → confidence ได้ถึง HIGH' : '❌ ขัดกัน → สวนเทรนด์ได้เฉพาะมี Structure reversal ชัดเจน (cap MEDIUM)'}
            ...
            - รูปแบบกราฟ: เกิด ${m15Data.candlestick_pattern} (Body = $${m15Data.candle_body_size}, เกณฑ์ reversal ขั้นต่ำ = $${(m15Data.atr * 0.5).toFixed(2)})
            ...
            เกณฑ์ confidence:
            - HIGH = ตามเทรนด์ + Structure ยืนยัน + ADX > 25
            - MEDIUM = สวนเทรนด์ที่มี reversal pattern หรือ ตามเทรนด์แต่ ADX 20-25
            - LOW = สัญญาณอ่อน, ขัดแย้ง, ไม่มี Structure ยืนยัน, หรือ Sideway ไม่ชัด
        `;
        ...
        const naiveTpDistance = slDistance * 2.0; // R:R เริ่มต้น 1:2.0
        let tpAdjustReason = '';
        ...
        if (parsedData.action === 'BUY') {
            ...
            if (resistanceWalls.length > 0) {
                ...
                if (adjustedRR >= 1.0) {
                    tpPrice = adjustedTp;
                    tpAdjustReason = `⚠️ TP ถูกดึงกลับจาก R:R 1:2.0 → 1:${adjustedRR.toFixed(2)} เพราะเจอ ${nearestWall.label} ที่ $${nearestWall.price.toFixed(2)} ขวาง`;
                } else {
