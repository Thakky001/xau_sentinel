...
export async function runScoutBot(testMode = false) {
    ...
    try {
        ...
        // 5️⃣ MACD Momentum & Divergence
        if (m15Data.macd) {
            if (m15Data.macd.state.includes('Cross')) {
                triggerReasons.push(`MACD ตัดกัน: ${m15Data.macd.state}`);
            } else if (m15Data.macd.state.includes('Divergence') || m15Data.macd.state.includes('แผ่ว')) {
                triggerReasons.push(`แจ้งเตือนโมเมนตัม MACD: ${m15Data.macd.state}`);
            }
        }
...
ตอบกลับเฉพาะรูปแบบ JSON ที่กำหนด ห้ามมีข้อความอื่นนอกกรอบ JSON เด็ดขาด (ยกเว้นแท็ก <think>)`;
        ...
        const hasMACD     = triggerReasons.some(t => t.includes('MACD'));

        if (hasRSI || triggerReasons.length === 0) guides.push(
            `[RSI สุดโต่ง] ราคาลงมาชนแนวรับ/ขึ้นไปชนแนวต้านไหม? เทรนด์ใหญ่ H1 สวนทางไหม?`
        );
        if (hasFibo) guides.push(
            `[Fibo Test] ราคาเด้งจากแนว Fibo หรือกำลังทะลุผ่าน? มีแท่งเทียนยืนยันการกลับตัวที่แนวนี้ไหม?`
        );
        if (hasPattern) guides.push(
            `[Price Action] Pattern เกิดที่แนวรับ/ต้านสำคัญไหม? สอดคล้องกับเทรนด์ H1 ไหม?`
        );
        if (hasBreakout) guides.push(
            `[Breakout] Breakout จริงหรือหลอก? ราคาปิดยืนเหนือ/ใต้แนวที่ทะลุหรือยัง?`
        );
        if (hasMACD || triggerReasons.length === 0) guides.push(
            `[MACD Momentum] ขัดแย้งกับกราฟราคาหรือไม่ (Divergence)? ถ้าแรงหดตัวให้ระวัง Fakeout`
        );
        ...
        const prompt = `
            ...
            วิเคราะห์ตามกรอบนี้: ${guides.join(' | ')}
            สรุป: พิจารณา Market Structure ควบคู่กับเหตุผลทั้งหมด ถ้าขัดแย้งให้ WAIT
            </think>

            ส่วนที่ 2: บทสรุปแผนการเทรด (ตอบเป็น JSON Format เท่านั้น ห้ามพิมพ์ข้อความอื่นต่อท้าย)
            {
        ...
        // 8. จัดหน้าตาข้อความส่งเข้า Telegram ใหม่ให้สวยงาม
        let messageToTelegram = `
🎯 <b>XAU_Sentinel AI Signal</b> 🎯
🌍 <b>แนวโน้ม:</b> ${m15Data.market_structure}
💡 <b>Action:</b> <b>${parsedData.action}</b>
📍 <b>Entry:</b> ${parsedData.entry}
🛑 <b>SL:</b> ${parsedData.sl}
💰 <b>TP:</b> ${parsedData.tp}
🛡️ <b>BE (บังทุน):</b> ${parsedData.be || 'ไม่ระบุ'}

📝 <b>เหตุผลจาก AI:</b> 
<i>${parsedData.reasoning}</i>

⚠️ <i>ราคากราฟตอนวิเคราะห์: $${m15Data.price}</i>
        `.trim();
        ...
        // 9. เริ่มนับเวลา Cooldown + บันทึก state
