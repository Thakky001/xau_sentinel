...
ตอบเป็นภาษาไทย ใช้ HTML format (<b>, <i>, <pre>) ห้ามใช้ Markdown ** เด็ดขาด`;
        ...
        // 5. ให้ AI วิเคราะห์ (ระบบมี Fallback สลับไปใช้ Groq ถ้า Gemini ล่ม)
        let aiResponse;
        let usedFallback = false;
        try {
            aiResponse = await fetchWithRetry(() => analyzeTrading(prompt, systemInstruction));
        } catch (geminiErr) {
            console.warn(`[Fallback] Gemini ล่มหรือติดลิมิต (${geminiErr.message})... กำลังสลับไปใช้ Groq AI สำรอง!`);
            try {
                // ให้ Groq ลองพยายามสัก 3 รอบถ้าพัง
                aiResponse = await fetchWithRetry(() => analyzeTradingWithGroq(prompt, systemInstruction), 3);
                usedFallback = true;
            } catch (groqErr) {
                throw new Error(`ทั้ง Gemini และ Groq ล่มทั้งหมด: ${groqErr.message}`);
            }
        }

        if (usedFallback) {
            aiResponse += '\n\n*(วิเคราะห์โดย ⚡ Groq Fallback AI เนื่องาก Google ขัดข้อง)*';
        }
