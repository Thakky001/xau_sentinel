...
export async function runScoutBot(testMode = false) {
    ...
    try {
        ...
        // ── Multi-Gate: ต้องมี >= 2 Cluster ที่ต่างกัน ถึงจะปลุก AI ──
        if (!testMode && activeClusters.size < 2) {
            console.log(`💤 ข้ามการวิเคราะห์รอบนี้ — ผ่านแค่ ${activeClusters.size} Cluster (ต้องการ 2):`);
...
ตอบกลับเฉพาะรูปแบบ JSON ที่กำหนด ห้ามมีข้อความอื่นนอกกรอบ JSON เด็ดขาด (ยกเว้นแท็ก <think>)`;
        ...
        if (botState.lastAnalysis) {
            ...
            - รอบก่อน AI ตัดสินใจ: ${la.decision || la.action || 'N/A'} เพราะ "${la.reasoning || 'ไม่มีข้อมูล'}"
            - ราคาตอนนั้น: $${la.price || 'N/A'} → ตอนนี้: $${m15Data.price} (${direction} $${Math.abs(priceChange)})
            - ⚠️ ถ้าสถานการณ์ไม่เปลี่ยนแปลงอย่างมีนัยสำคัญ ให้ยืนยันการตัดสินใจเดิม อย่าเปลี่ยนใจไปมา`;
        }
        ...
        if (parsedData.action === 'BUY') {
            ...
            if (resistanceWalls.length > 0) {
                ...
                if (adjustedRR >= 1.0) {
                    ...
                    botState.lastAnalysis = {
                        action: 'WAIT', price: m15Data.price, time: new Date().toISOString(),
