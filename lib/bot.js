...
import { addOrderToTracker } from './tracker.js';
...
ตอบกลับเฉพาะรูปแบบ JSON ที่กำหนด ห้ามมีข้อความอื่นนอกกรอบ JSON เด็ดขาด (ยกเว้นแท็ก <think>)`;
        ...
        const entryPrice = parseFloat(m15Data.price);
        const currentAtr = m15Data.atr || 2.0; // ค่า Default ถ้าหา ATR ไม่ได้
        
        let slPrice = 0;
        let tpPrice = 0;
        let bePrice = 0;
        const spreadBuffer = 0.5; // เผื่อ Spread 50 pips

        if (parsedData.action === 'BUY') {
            slPrice = entryPrice - (currentAtr * 1.5);
            const slDistance = entryPrice - slPrice;
            tpPrice = entryPrice + (slDistance * 1.5); // Risk:Reward 1:1.5
            bePrice = entryPrice + spreadBuffer + (currentAtr * 0.8); // บังทุนเมื่อกราฟวิ่งไปเกินครึ่งทางของ TP
        } else if (parsedData.action === 'SELL') {
            slPrice = entryPrice + (currentAtr * 1.5);
            const slDistance = slPrice - entryPrice;
            tpPrice = entryPrice - (slDistance * 1.5); // Risk:Reward 1:1.5
            bePrice = entryPrice - spreadBuffer - (currentAtr * 0.8);
        }
