...
export async function runScoutBot(testMode = false) {
    ...
    try {
        ...
        // ── Multi-Gate: ต้องมี >= 1 Cluster ถึงจะปลุก AI ──
        if (!testMode && activeClusters.size < 1) {
            console.log(`💤 ข้ามการวิเคราะห์รอบนี้ — ไม่ผ่านเงื่อนไขสัก Cluster (ต้องการ 1):`);
...
2. ห้าม counter-trend: BUY ต้อง H1 uptrend + M15 HH/HL | SELL ต้อง H1 downtrend + M15 LH/LL
3. RSI oversold ใน downtrend = falling knife → WAIT ไม่ใช่ BUY
4. RSI overbought ใน uptrend = FOMO → WAIT ไม่ใช่ SELL
5. ผ่านแค่ 1 Cluster → WAIT (ยกเว้นมี Structure cluster ร่วม)
6. ADX < 20 → ตลาด Sideway ไร้ทิศทางชัดเจน → WAIT
7. สัญญาณขัดกัน → WAIT ทันที
