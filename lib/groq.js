...
export async function analyzeTradingWithGroq(prompt, systemInstruction = "คุณคือ AI ผู้เชี่ยวชาญด้านการเทรดทองคำ (XAUUSD) ให้คำแนะนำที่กระชับ แม่นยำ และจัดรูปแบบข้อความให้อ่านง่ายสำหรับ Telegram ห้ามตอบเรื่องอื่นที่ไม่เกี่ยวกับการเทรด") {
    ...
    console.log('Requesting analysis from Groq (Primary)...');
    ...
    const response = await groq.chat.completions.create({
        ...
        model: "llama-3.3-70b-versatile", // อัปเกรดเป็นรุ่นท็อป 70b เพื่อความฉลาดสูงสุด
        temperature: 0.15, // ต่ำ = สม่ำเสมอ เหมาะกับการตัดสินใจเงิน
        max_tokens: 800, // บีบเพดานคำไว้ไม่ให้เกินโควต้า 6,000 TPM ของของฟรี
        top_p: 1,
    });
