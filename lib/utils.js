...
export async function fetchWithRetry(fn, retries = 5, baseDelayMs = 5000) {
    for (let i = 0; i <= retries; i++) {
        try {
            return await fn();
        } catch (err) {
            // ไม่ต้อง retry สำหรับ Error ที่ retry ไปก็ไม่ช่วย (เช่น ถูกบล็อก, Token ผิด, โดนแบน, ติด Rate Limit)
            const isNonRetryable = /blocked:|GEMINI_API_KEY|Unauthorized|401|403|429|rate limit|rate_limit|Rate Guard/i.test(err.message);
            if (isNonRetryable || i === retries) throw err;
            
            // ใช้ Exponential Backoff: รอ 5s, 10s, 20s, 40s, 80s
