...
function processCandles(candles, mode = 'full') {
    ...
    // แบ่งครึ่ง 48 แท่งแรก กับ 48 แท่งหลังเพื่อเปรียบเทียบโครงสร้างราคา (Market Structure)
    const firstHalfHigh = Math.max(...highs.slice(-96, -48));
    const firstHalfLow = Math.min(...lows.slice(-96, -48));
    const secondHalfHigh = Math.max(...highs.slice(-48));
    const secondHalfLow = Math.min(...lows.slice(-48));
