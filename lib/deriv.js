    ...
    return new Promise((resolve, reject) => {
        ...
        ws.on('message', (data) => {
            ...
            if (response.msg_type === 'candles' && response.candles) {
                if (response.candles.length < 34) {
                    return finish(reject, new Error(`Insufficient candles: got ${response.candles.length}, need at least 34 for MACD calculation`));
                }
                return finish(resolve, processCandles(response.candles));
            }
 ...
 */
function detectCandlestickPattern(latest, prev) {
 ...
 */
function calcFibonacci(recentHigh, recentLow) {
